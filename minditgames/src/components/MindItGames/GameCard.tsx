...
import { Star, Download, Gamepad2, Sparkles, ChevronLeft, ChevronRight, Apple } from 'lucide-react'
...
const GameCard = ({ game, reverse = false }: GameCardProps) => {
  ...
  const prevShot = () =>
    setActiveShot((i) => (i - 1 + game.screenshots.length) % game.screenshots.length)
    ...
    >
      <div className={`grid lg:grid-cols-2 ${reverse ? 'lg:[direction:rtl]' : ''}`}>
        ...
        <div className="relative lg:[direction:ltr]">
          ...
          {/* Screenshot strip overlay */}
          <div className="absolute inset-x-4 bottom-4 flex items-center gap-2 lg:inset-x-6 lg:bottom-6">
            <div className="flex-1 overflow-hidden rounded-xl border border-white/10 bg-black/40 backdrop-blur-md">
              <div className="flex items-center gap-2 p-2">
                <button
                  onClick={prevShot}
                  aria-label="Previous screenshot"
                  className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-white/10 text-white transition-colors hover:bg-white/20"
                >
                  <ChevronLeft size={14} />
                </button>
                ...
                <PlaceholderImage
                  ...
                  alt={game.screenshots[activeShot]?.alt ?? `${game.title} screenshot`}
                  ...
                  {game.screenshots.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all ${
                        i === activeShot ? 'w-5 bg-white' : 'w-1.5 bg-white/30'
                      }`}
                    />
                  ))}
                </div>
        ...
        <div className="flex flex-col p-8 lg:p-10 lg:[direction:ltr]">
            ...
            <div>
              <span
                className={`text-xs font-bold uppercase tracking-wider ${
                  game.accent === 'orange' ? 'text-brand-orange-soft' : 'text-brand-blue-soft'
                }`}
              >
                {game.category}
              </span>
              <div className="mt-1 flex items-center gap-1.5">
                ...
                <span className="ml-1 text-xs font-semibold text-slate-300">{game.rating}</span>
                <span className="text-xs text-slate-500">({game.ratingCount})</span>
              </div>
            </div>
          </div>
          ...
          <p className="mt-2 text-sm font-medium text-slate-400">{game.tagline}</p>

          {/* Description */}
          <p className="mt-5 text-sm leading-relaxed text-slate-400">{game.description}</p>
              ...
              <li key={i} className="flex items-start gap-2.5 text-sm text-slate-300">
                <span
                  className={`mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full ${
                    game.accent === 'orange' ? 'bg-brand-orange' : 'bg-brand-blue'
                  }`}
                />
                {feature}
              </li>
